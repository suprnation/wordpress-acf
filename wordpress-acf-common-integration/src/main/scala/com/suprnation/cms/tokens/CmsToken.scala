...
trait CmsFieldTokenWithPostType[T <: CmsPostIdentifier] extends CmsFieldToken with PostType {
  val parameterisedType: Class[T]
}


object PostToken {
  def apply[T <: CmsPostIdentifier](postType: String, source: Class[T]) = new PostToken[T](postType, source, List.empty)
}

case class PostToken[T <: CmsPostIdentifier](postType: String, source: Class[T], fields: List[CmsFieldToken]) extends CmsToken with PostType {
...
case class PostFieldToken[T <: CmsPostIdentifier](postType: String, injector: Injector, postToken: PostToken[T]) extends CmsFieldTokenWithPostType[T] {
